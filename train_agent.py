...
class PseudoDemoGenerator:
    ...
    def __init__(self, device, num_demos = 5, min_num_waypoints = 2, max_num_waypoints = 6, sample_rate = 5):
    ...
    def get_sample(self):
        ...
        curr_obs, clean_actions, fututre_obs = self._get_ground_truth()
    ...
    def _run_game(self,biased, augmented):
        ...
        for attempt in range(max_retries):
            try: 
                pseudo_demo = PseudoGame(max_num_sampled_waypoints=self.max_num_waypoints, min_num_sampled_waypoints=self.min_num_waypoints, biased=biased, augmented=augmented)
    ...
    def _get_context(self):
        ...
        for _ in range(self.num_demos - 1):
            biased = True
            augemented = False
            pseudo_demo = self._run_game(biased=biased, augmented=augemented)
            observations = pseudo_demo.observations
            sampled_obs = observations[::self.sample_rate]  # downsample observations with self.sample_rate
    ...
    def _get_ground_truth(self):
        ...
        actions = torch.tensor(pseudo_demo.get_actions(mode='vector', angle_unit='rad'), dtype=torch.float, device = self.device)          
...
class Trainer: 
    ...
    def train_step(self):
        ...
        for i in range(self.batch_size):
            curr_obs, context, clean_actions = self.data_generator.get_sample()
