...
class Action:
    ...
    def as_vector(self, mode = 'deg'):
        ...
        vector[0] = np.cos(theta) * self.forward_movement
        vector[1] = np.sin(theta) * self.forward_movement
        vector[2] = self.rotation if mode == 'deg' else theta
        vector[3] = self.state_change.value
        return vector 
...
class Player:
    ...
    def move_with_action(self,action : Action):
        ...
        state_change_action = movement_vector[3]
        angle = movement_vector[2]
