...
class ParallelPseudoDemoGenerator:
...
class BatchedTrainer: 
    def __init__(self, agent, num_demos_for_context, min_demo_length=2, max_demo_length=6, 
                 batch_size=10, device='cuda', num_threads=4):
        self.agent = agent
        self.device = device 
        self.data_generator = ParallelPseudoDemoGenerator(
    ...
    def train_step(self):
        """Batched training step - no more for loop!"""
        ...
        # Forward pass through agent with batched inputs
        predicted_noise, actual_noise = self.agent.forward_batch(
            curr_obs_batch, context_batch, clean_actions_batch
        )
        
        # Both should be [batch_size, pred_horizon, 4] 
        assert predicted_noise.shape == actual_noise.shape, \
            f"Shape mismatch: {predicted_noise.shape} vs {actual_noise.shape}"
    ...
    def full_training(self, num_steps_per_epoch=200, num_epochs=50, 
        ...
        for epoch in range(num_epochs):
            ...
            # Learning rate cooldown in final 4 epochs
        ...
        if save_model:
            ...
            # Fixed: save alpha_schedule from agent, not trainer
            torch.save(self.agent.alpha_schedule, f'{save_path[:-4]}_alpha_schedule.pth')
...
if __name__ == "__main__":
    ...
    # Initialize agent (you'll need to implement forward_batch method)
