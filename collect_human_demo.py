...
def collect_human_demos():
    """Collect human demonstrations across different configurations."""
    
    for i in range(NUM_KIND_OF_DEMO):
        print(f"\n{'='*50}")
        print(f"Collecting DEMOSET {i}/{NUM_KIND_OF_DEMO}")
        print(f"{'='*50}")
...
def replay_demo_with_config(demoset_id, demo_id):
    ...
    if game_interface.set_initial_config(config_path + '.json'):
        ...
        if demo:
            ...
            # Here you would implement the replay logic based on your observations
...
if __name__ == "__main__":
    ...
    # Replay a demo with its configuration
